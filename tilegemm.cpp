                                                          ...
                                                          int *d_spec_intersection_cnt, int *d_spec_intersection_posa, int *d_spec_intersection_posb)
{
    ...
    for (int i = lane_id; i < nmasks_warpwise; i += WARP_SIZE)
    {
        ...
        // sum
        // 将cnt_scan变量在warp内的第31个线程中的值广播给warp内的其他线程
    ...
    for (int ri = 0; ri < REPEAT_NUM; ri++)
    {
        ...
        *time_malloc += (t2.tv_sec - t1.tv_sec) * 1000.0 + (t2.tv_usec - t1.tv_usec) / 1000.0;
        ...
        else
        {
            ...
            int num_blocks = ceil((float)blkmA / (float)(WARP_PER_BLOCK));
...
// -------------------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        *time_malloc += (t2.tv_sec - t1.tv_sec) * 1000.0 + (t2.tv_usec - t1.tv_usec) / 1000.0;
        ...
        if (densityA > INTERSECTION_SPARSE_OR_DNS_TH && densityB > INTERSECTION_SPARSE_OR_DNS_TH)
        {
            ...
            {
                int num_threads = WARP_SIZE * WARP_PER_BLOCK;
                int num_blocks = ceil((float)numblkC / (float)num_threads);
            ...
            else
            {
                ...
                num_blocks = ceil((float)numblkC / (float)(HALFWARP_PER_BLOCK * TILE_PER_HALFWARP));
        ...
        else
        {
            ...
            else
            {
                ...
                num_blocks = ceil((float)numblkC / (float)(WARP_PER_BLOCK * TILE_PER_WARP));
        ...
        *time_malloc += (t2.tv_sec - t1.tv_sec) * 1000.0 + (t2.tv_usec - t1.tv_usec) / 1000.0;
        ...
        {
            num_threads = HALFWARP_SIZE * HALFWARP_PER_BLOCK;
            num_blocks = ceil((float)blksmem_tny_cnt / (float)HALFWARP_PER_BLOCK);
        ...
        {
            num_threads = WARP_SIZE * WARP_PER_BLOCK;
            num_blocks = ceil((float)blksmem_sml_cnt / (float)WARP_PER_BLOCK);
        ...
        {
            num_threads = WARP_SIZE * WARP_PER_BLOCK;
            num_blocks = ceil((float)blksmem_lrg_cnt / (float)WARP_PER_BLOCK);
        ...
        {
            num_threads = HALFWARP_SIZE * HALFWARP_PER_BLOCK;
            num_blocks = ceil((float)blksmem_dns_cnt / (float)HALFWARP_PER_BLOCK);
        ...
        {
            num_threads = HALFWARP_SIZE * HALFWARP_PER_BLOCK;
            num_blocks = ceil((float)blksmem_ful_cnt / (float)HALFWARP_PER_BLOCK);
        ...
        *time_step3 = (t2.tv_sec - t1.tv_sec) * 1000.0 + (t2.tv_usec - t1.tv_usec) / 1000.0;
        ...
        float time = (tend.tv_sec - tstart.tv_sec) * 1000.0 + (tend.tv_usec - tstart.tv_usec) / 1000.0;
        time_all[ri] = time;
